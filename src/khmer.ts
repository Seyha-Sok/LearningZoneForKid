...
const hasNumberedGuide = (_letter:string) => group !== 'independent';
...
<div class="workspace"><aside class="alphabet-panel"><div class="section-label"><h2>Pick a character</h2><span id="character-count">33 consonants</span></div><label class="character-group-label" for="character-group">Choose a group</label><select id="character-group"><option value="consonants">Consonants · ព្យញ្ជនៈ</option><option value="vowels">Vowel signs · ស្រៈនិស្ស័យ</option><option value="independent">Independent vowels · ស្រៈពេញតួ</option></select><div id="alphabet" class="alphabet-grid" aria-label="Khmer consonants"></div><div class="encouragement"><span>🌱</span><p>Take your time.<br><strong>Every try helps.</strong></p></div></aside>
...
function render() {
  ...
  el('audio-status').textContent=hasKhmerAudio(letter)?'':(group==='independent'?'New recording coming soon.':'Audio not available yet for this character.');
  ...
  el('letter-title').textContent=mode==='trace'?(group==='consonants'?'Trace a Khmer character':'Trace a Khmer vowel'):'Your free drawing space';
  ...
  el('board-label').textContent=mode==='trace'?(numbered?'FOLLOW THE NUMBERED ARROWS':'FOLLOW THE VOWEL SHAPE'):'MAKE YOUR OWN LITTLE MARKS';
  ...
  el('feedback-text').textContent=mode==='trace'?(group==='vowels'?'The dotted circle stands for a consonant. Trace only the vowel marks.':numbered?'Follow the arrows slowly. The numbers show the order.':'Follow the shape slowly. This is a shape-only guide.'):'This space is yours. Have fun drawing!';
}
function renderChoices() {
 ...
 characters.forEach((letter,i)=>{const b=document.createElement('button');b.lang='km';b.textContent=displayCharacter(letter);b.setAttribute('aria-label',`Khmer ${group==='consonants'?'character':'vowel'} ${displayCharacter(letter)}`);b.onclick=()=>{selected=i;render();};el('alphabet').append(b);});
}
