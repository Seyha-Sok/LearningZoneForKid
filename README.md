...
The Khmer group dropdown also includes 23 vowel-sign forms (common combinations included) and 13 independent-vowel practice forms following the supplied reference (ending ឱ, ឳ, ឲ្យ). 22 vowel signs have locally bundled, attributed worksheet direction guides. The independent vowels currently use pale shape-only references, explicitly labeled without stroke directions. A dotted circle indicates where a consonant belongs and is not part of the vowel. Group changes clear the current drawing. No AI scoring or vowel audio was added.
